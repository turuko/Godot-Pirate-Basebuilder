...
func _process(_delta):
	...
	if GameManager.instance.bmc._build_mode == BuildModeController.BuildMode.FIXTURE and !_is_dragging:
...
func _update_dragging():
	...
	if Input.is_action_pressed("left_mouse"):
		if GameManager.instance.bmc._build_mode == BuildModeController.BuildMode.FIXTURE:
			offset = Vector2(GameManager.instance.map_controller.map._fixture_prototypes[GameManager.instance.bmc._build_mode_type]._width, GameManager.instance.map_controller.map._fixture_prototypes[GameManager.instance.bmc._build_mode_type]._height)
			for x in range(start_x, end_x+1, offset.x):
				for y in range(start_y, end_y+1, offset.y):
					...
					if t != null and t._fixture == null and t._job == null:
						...
						if GameManager.instance.bmc._build_mode == BuildModeController.BuildMode.FIXTURE:
							node.texture = _textures[GameManager.instance.bmc._build_mode_type]
		...
		if GameManager.instance.bmc._build_mode == BuildModeController.BuildMode.FIXTURE:
			for x in range(start_x, end_x+1, offset.x):
				for y in range(start_y, end_y+1, offset.y):
					...
					var t = GameManager.instance.map_controller.map.get_tile_at(x,y)
					if t != null and t._job == null:
						print("call build from mouse controller")
						GameManager.instance.bmc.build([t])
		elif GameManager.instance.bmc._build_mode == BuildModeController.BuildMode.ZONE:
